...
**AI 코딩 어시스턴트를 위한 스킬.** Claude Code, Codex, OpenCode, OpenClaw, Factory Droid, Trae, 또는 Cursor에서 `/graphify`를 입력하면 파일을 읽고 지식 그래프를 구축하여, 미처 몰랐던 구조를 보여줍니다. 코드베이스를 더 빠르게 이해하고, 아키텍처 결정의 "이유"를 찾아보세요.
...
**필수 요구사항:** Python 3.10+ 및 다음 중 하나: [Claude Code](https://claude.ai/code), [Codex](https://openai.com/codex), [OpenCode](https://opencode.ai), [OpenClaw](https://openclaw.ai), [Factory Droid](https://factory.ai), [Trae](https://trae.ai), 또는 [Cursor](https://cursor.com)
...
**OpenCode**는 `AGENTS.md`에 작성하고 bash 도구 호출 전에 실행되는 **`tool.execute.before` 플러그인**(`.opencode/plugins/graphify.js` + `opencode.json` 등록)을 설치하며, 그래프가 있을 때 도구 출력에 그래프 알림을 주입합니다.

**OpenClaw, Factory Droid, Trae**는 프로젝트 루트의 `AGENTS.md`에 동일한 규칙을 작성합니다. 이 플랫폼들은 도구 훅을 지원하지 않으므로 AGENTS.md가 상시 작동 메커니즘입니다.

**Cursor**는 두 가지를 수행합니다: 아키텍처 질문에 답하기 전에 `graphify-out/GRAPH_REPORT.md`를 읽도록 지시하는 `.cursor/rules/graphify.mdc` 규칙 파일(`alwaysApply: true`)을 작성하고, 새 에이전트 세션이 시작될 때 실행되는 **sessionStart 훅**을 설치합니다. 지식 그래프가 존재하면 Cursor는 동일한 안내를 보게 됩니다 — 모든 파일을 grep하는 대신 그래프를 통해 탐색합니다.
...
graphify cursor install            # .cursor/rules/ + sessionStart 훅 (Cursor)
