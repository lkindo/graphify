...
**Hierarchical clustering — no embeddings.** Two-pass Leiden organizes nodes into L0 topics → L1 communities → L2 detail nodes. The first pass clusters nodes into communities by edge density; the second pass groups related communities into broader topics by running Leiden on the contracted community graph. Queries can drill down from topics to communities to individual nodes — navigation cost stays logarithmic as the knowledge base scales. The semantic similarity edges that Claude extracts (`semantically_similar_to`, marked INFERRED) influence clustering directly since they're part of the graph topology.
