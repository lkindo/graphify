...
def absolute_verified_extraction():
    ...
    # TOC mapping based on the user's physical proof (Page 384 = Ttak-chong-na-mu)
    mapping = [
    ...
    for item in mapping:
        ...
        # Rule: Page X in TOC -> PDF index X (which is page X+1)
        p_idx = item["page"]
        
        if p_idx < len(doc):
            pix = doc[p_idx].get_pixmap(matrix=fitz.Matrix(2, 2))
            pix.save(os.path.join(output_dir, f"{name}_1.jpg"))
            
        if p_idx + 1 < len(doc):
            pix = doc[p_idx + 1].get_pixmap(matrix=fitz.Matrix(2, 2))
            pix.save(os.path.join(output_dir, f"{name}_2.jpg"))

    doc.close()
    print("Extraction complete based on physical page numbers.")

if __name__ == "__main__":
    absolute_verified_extraction()
