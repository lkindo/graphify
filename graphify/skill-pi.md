...
**Step B2 - Sequential extraction (Pi)**
...
## For native Pi integration

Run once per project to make graphify always-on in Pi sessions:

```bash
graphify pi install
```

This installs the Pi graphify skill, writes a `## graphify` section to the local `AGENTS.md`, and adds a `.pi/prompts/graphify.md` prompt alias so `/graphify` works inside the project. No manual `/skill:graphify` needed in future sessions.

```bash
graphify pi uninstall  # remove the AGENTS.md section and /graphify prompt alias
```
