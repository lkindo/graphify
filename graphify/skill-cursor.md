...
**MANDATORY: You MUST dispatch subagents in parallel here. Reading files yourself one-by-one is forbidden - it is 5-10x slower. If you do not dispatch subagents in parallel you are doing this wrong.**
...
Dispatch multiple subagents IN THE SAME RESPONSE - one call per chunk. This is the only way they run in parallel. If you make one subagent call, wait, then make another, you are doing it sequentially and defeating the purpose.

Concrete example for 3 chunks:
```
[Subagent call 1: files 1-15]
[Subagent call 2: files 16-30]  
[Subagent call 3: files 31-45]
```
...
## For native Cursor rules integration

Run once per project to make graphify always-on in Cursor sessions:

```bash
graphify cursor install
```

This writes a `.cursor/rules/graphify.mdc` rule file that instructs Cursor Agent to check the graph before answering codebase questions and rebuild it after code changes. No manual `/graphify` needed in future sessions.

```bash
graphify cursor uninstall  # remove the section
```
