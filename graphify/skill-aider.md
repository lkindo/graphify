...
/graphify <path> --max-nodes 10000                    # raise HTML viz node limit (default 5000; performance may degrade above 5000)
...
# Use --max-nodes value if provided, otherwise default to 5000
max_nodes = 5000  # replace with --max-nodes value if given
if max_nodes > 5000:
    print(f'warning: --max-nodes={max_nodes} exceeds recommended limit of 5000. Performance may degrade.', file=__import__('sys').stderr)
if G.number_of_nodes() > max_nodes:
    print(f'Graph has {G.number_of_nodes()} nodes - too large for HTML viz (limit: {max_nodes}). Use Obsidian vault instead or pass --max-nodes with a higher value.')
else:
    to_html(G, communities, 'graphify-out/graph.html', community_labels=labels or None, max_nodes=max_nodes)
...
- Never run HTML viz on a graph with more than 5,000 nodes without warning the user. If the user passes `--max-nodes` with a value above 5,000, warn that browser performance may degrade.
