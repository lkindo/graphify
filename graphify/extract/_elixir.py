"""Elixir language extractor (custom walk — not driven by LanguageConfig)."""
...
def extract_elixir(path: Path) -> dict:
    ...
    try:
        ...
        return make_error_result("tree_sitter_elixir not installed")
    ...
    def walk(node, parent_module_nid: str | None = None) -> None:
        ...
        if keyword == "defmodule":
            ...
            if not module_name:
                return
            module_nid = _make_id(ctx.stem, module_name)
            ctx.add_node(module_nid, module_name, line)
            ctx.add_edge(ctx.file_nid, module_nid, "contains", line)
            if do_block_node:
        ...
        if keyword in ("def", "defp"):
            ...
            if not func_name:
                return
            container = parent_module_nid or ctx.file_nid
            func_nid = _make_id(container, func_name)
            ctx.add_node(func_nid, f"{func_name}()", line)
            if parent_module_nid:
                ctx.add_edge(parent_module_nid, func_nid, "method", line)
            else:
                ctx.add_edge(ctx.file_nid, func_nid, "contains", line)
            if do_block_node:
                ctx.function_bodies.append((func_nid, do_block_node))
            return
    ...
    for caller_nid, body in ctx.function_bodies:
        walk_calls(body, caller_nid)

    clean_edges = [e for e in ctx.edges if e["source"] in ctx.seen_ids and
                   (e["target"] in ctx.seen_ids or e["relation"] == "imports")]
    return {"nodes": ctx.nodes, "edges": clean_edges, "input_tokens": 0, "output_tokens": 0}
