...
/graphify <path> --update                             # incremental - re-extract only new/changed files, prune deleted nodes
/graphify <path> --update --no-prune                 # incremental - skip pruning (keep nodes from deleted files)
...
Note whether `--no-prune` was given. Replace `NO_PRUNE` with `True` in the merge block below if it was, otherwise `False`.
...
# Prune nodes from deleted files before merging (skip if --no-prune was given)
no_prune = NO_PRUNE
if not no_prune:
    ...
    print('[graphify] --no-prune: skipping ghost node removal')
