...
/graphify <path> --output-dir <dir>                   # write all outputs to <dir> instead of graphify-out
...
cached_nodes, cached_edges, cached_hyperedges, uncached = check_semantic_cache(all_files, output_dir=Path('${GRAPHIFY_OUT}'))
...
saved = save_semantic_cache(new.get('nodes', []), new.get('edges', []), new.get('hyperedges', []), output_dir=Path('${GRAPHIFY_OUT}'))
...
to_json(G, communities, '${GRAPHIFY_OUT}/graph.json')
...
- If `--obsidian-dir <path>` was also given, use that path as the vault directory. Otherwise default to `${GRAPHIFY_OUT}/obsidian`.
...
obsidian_dir = 'OBSIDIAN_DIR'  # replace with --obsidian-dir value, or '${GRAPHIFY_OUT}/obsidian' if not given
...
if G.number_of_nodes() > 5000:
    ...
    to_html(G, communities, '${GRAPHIFY_OUT}/graph.html', community_labels=labels or None)
...
to_cypher(G, '${GRAPHIFY_OUT}/cypher.txt')
print('cypher.txt written - import with: cypher-shell < ${GRAPHIFY_OUT}/cypher.txt')
"
```
...
to_svg(G, communities, '${GRAPHIFY_OUT}/graph.svg', community_labels=labels or None)
...
to_graphml(G, communities, '${GRAPHIFY_OUT}/graph.graphml')
...
```json
{
  "mcpServers": {
    "graphify": {
      "command": "python3",
      "args": ["-m", "graphify.serve", "/absolute/path/to/your-output-dir/graph.json"]
    }
  }
}
```
...
detection = json.loads(Path('.graphify_detect.json').read_text())
result = run_benchmark('${GRAPHIFY_OUT}/graph.json', corpus_words=detection['total_words'])
...
detect = json.loads(Path('.graphify_detect.json').read_text())
save_manifest(detect['files'], manifest_path='${GRAPHIFY_OUT}/manifest.json')
...
cost_path = Path('${GRAPHIFY_OUT}/cost.json')
if cost_path.exists():
...
result = detect_incremental(Path('INPUT_PATH'), manifest_path='${GRAPHIFY_OUT}/manifest.json')
...
# Load existing graph
existing_data = json.loads(Path('${GRAPHIFY_OUT}/graph.json').read_text())
...
Before the merge step, save the old graph: `cp "${GRAPHIFY_OUT}/graph.json" .graphify_old.json`
...
Skip Steps 1–3. Load the existing graph from `${GRAPHIFY_OUT}/graph.json` and re-run clustering:
...
data = json.loads(Path('${GRAPHIFY_OUT}/graph.json').read_text())
...
$(cat .graphify_python) -c "
from pathlib import Path
if not Path('${GRAPHIFY_OUT}/graph.json').exists():
...
Load `${GRAPHIFY_OUT}/graph.json`, then:
...
print('Query result saved to ${GRAPHIFY_OUT}/memory/')
"
```
...
print('Path result saved to ${GRAPHIFY_OUT}/memory/')
"
```
...
print('Explanation saved to ${GRAPHIFY_OUT}/memory/')
"
```
...
```bash
python3 -m graphify.watch INPUT_PATH --debounce 3 --output-dir "${GRAPHIFY_OUT}"
```
