...
def main() -> None:
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help"):
        ...
        print()
        print("Commands:")
        print("  install [--platform P]  copy skill to platform config dir (claude|windows|codex|opencode|ob1|claw|droid)")
        ...
        print("  ob1 install            write graphify section to AGENTS.md (OB-1, CC Compatible)")
    ...
    if cmd == "install":
        ...
        if subcmd == "install":
            ...
            sys.exit(1)
    elif cmd in ("codex", "opencode", "ob1", "claw", "droid"):
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
