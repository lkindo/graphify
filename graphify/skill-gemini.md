...
This writes a `## graphify` section to the local `GEMINI.md` that instructs Gemini to check the graph before answering codebase questions and rebuild it after code changes. It also installs a `BeforeTool` hook in `.gemini/settings.json` that fires before every file-read tool call — if a knowledge graph exists, Gemini sees a reminder to navigate via the graph instead of reading raw files. No manual `/graphify` needed in future sessions.

```bash
graphify gemini uninstall  # remove the section and hook
```
