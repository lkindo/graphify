...
Three things it does that Gemini alone cannot:
...
Use `run_shell_command` to detect and install graphify:
...
This step has two parts: **structural extraction** (deterministic, free) and **semantic extraction** (Gemini, costs tokens).
...
> **Gemini platform:** Use the `generalist` subagent for parallel extraction. Dispatch multiple `generalist` calls in a single turn for parallel execution.
...
**Step B2 - Dispatch ALL subagents in a single turn**

Call the `generalist` tool multiple times IN THE SAME TURN - one call per chunk. Set `wait_for_previous: false` (default) to ensure they run in parallel.

Each subagent receive this exact prompt (substitute FILE_LIST, CHUNK_NUM, TOTAL_CHUNKS, and DEEP_MODE):
...
Wait for all `generalist` calls to finish. For each result:
...
This writes a `## graphify` section to the local `GEMINI.md` that instructs Gemini to check the graph before answering codebase questions and rebuild it after code changes.
