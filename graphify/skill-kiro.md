...
**MANDATORY: You MUST use the use_subagent tool here. Reading files yourself one-by-one is forbidden - it is 5-10x slower. If you do not use use_subagent you are doing this wrong.**

Before dispatching subagents, print a timing estimate:
- Load `total_words` and file counts from `graphify-out/.graphify_detect.json`
- Estimate agents needed: `ceil(uncached_non_code_files / 22)` (chunk size is 20-25)
- Estimate time: ~45s per batch of 4 (they run in parallel, so total ≈ 45s × ceil(agents/4))
...
**Step B2 - Dispatch subagents via use_subagent (Kiro CLI)**

> **Kiro CLI platform:** Uses `use_subagent` with `InvokeSubagents` command. Up to 4 subagents run in parallel per batch. If you have more than 4 chunks, dispatch in batches of 4 — wait for each batch to complete before dispatching the next.

Call `use_subagent` with `InvokeSubagents` — include up to 4 subagent entries in the `subagents` array. All subagents in a single call run in parallel.

Concrete example for 3 chunks (fits in one batch):
```
use_subagent(command="InvokeSubagents", content={"subagents": [
  {"query": "[extraction prompt for chunk 1]"},
  {"query": "[extraction prompt for chunk 2]"},
  {"query": "[extraction prompt for chunk 3]"}
]})
```

For 6 chunks (two batches of 3-4):
```
# Batch 1: chunks 1-4
use_subagent(command="InvokeSubagents", content={"subagents": [
  {"query": "..."}, {"query": "..."}, {"query": "..."}, {"query": "..."}
]})
# Wait for batch 1 results, then batch 2: chunks 5-6
use_subagent(command="InvokeSubagents", content={"subagents": [
  {"query": "..."}, {"query": "..."}
]})
```

Each subagent query must include the full extraction prompt AND a final instruction to write the result to disk:

```
After extracting, write the JSON result to graphify-out/.graphify_chunk_NN.json using the write tool (replace NN with the chunk number, zero-padded).
```

Subagents have access to `read`, `write`, `shell`, and `code` tools — they can read files and write chunk results to disk.
...
- If the file is missing, the subagent may have failed — print a warning: "chunk N missing from disk — subagent may have failed." Do not silently skip.
